 ...
 *
 * Rules:
 *  - Only jpg/png accepted, no size limit beyond the shared 10MB cap
...
router.post('/:projectId', (req, res) => {
  photoUpload.array('photos', MAX_PHOTOS_PER_TYPE)(req, res, async (err) => {
...
router.delete('/:photoId', async (req, res) => {
  ...
  if (role === 'ARTISAN') {
    ...
    const diffHours = (Date.now() - new Date(photo.uploaded_at)) / (1000 * 60 * 60);
    if (diffHours > 24) {
